...
def smiles2morgan_raw(s, radius=2, nBits=2048):
    try:
        ...
        logging.warning(f"Error in smiles2morgan_raw for SMILES {s}: {str(e)}")
        return None

def add_molecular_features(df, smiles_col='smiles', feature_types=None):
    """Add molecular features to DataFrame."""
    if feature_types is None:
        feature_types = ['morgan', 'pubchem', 'rdkit2d', 'maccs', 'daylight', 'physchem']
    # Define a function to calculate physical and chemical properties
    def calculate_properties(smiles):
        molecule = Chem.MolFromSmiles(smiles)
        if molecule is None:
            return pd.Series([None] * 10)  # Adjust the number as per properties you are calculating
        ...
        return pd.Series([logp, mol_wt, hbond_donor, hbond_acceptor, tpsa, num_rotatable_bonds,
                         num_aromatic_rings, num_heteroatoms, num_aliphatic_rings])

    # Check if 'physchem' is in the list of feature types
    if 'physchem' in feature_types:
        # Apply the calculate_properties function to the 'smiles' column
        properties = df[smiles_col].apply(calculate_properties)

        # Set column names for the properties DataFrame
        properties.columns = ['LogP', 'MolecularWeight', 'HbondDonor', 'HbondAcceptor', 'TPSA',
                              'NumRotatableBonds', 'NumAromaticRings',
                              'NumHeteroatoms', 'NumAliphaticRings']

        # Concatenate the original DataFrame with the generated properties DataFrame
    ...
    for feature_type in feature_types:
        if feature_type == 'morgan':
            for index, row in df.iterrows():
                ...
                features = smiles2morgan_raw(smiles)
                if features is not None:
                    for i, feature in enumerate(features):
                        df.at[index, f'morgan_{i}'] = feature
        elif feature_type == 'pubchem':
            for index, row in df.iterrows():
                smiles = row[smiles_col]
                features = smiles2pubchem(smiles)
                if features is not None:
                    for i, feature in enumerate(features):
                        df.at[index, f'pubchem_{i}'] = feature
        elif feature_type == 'rdkit2d':
            for index, row in df.iterrows():
                smiles = row[smiles_col]
                features = smiles2rdkit2d(smiles)
                if features is not None:
                    for i, feature in enumerate(features):
                        df.at[index, f'rdkit2d_{i}'] = feature
        elif feature_type == 'maccs':
            for index, row in df.iterrows():
                smiles = row[smiles_col]
                features = smiles2maccs(smiles)
                if features is not None:
                    for i, feature in enumerate(features):
                        df.at[index, f'maccs_{i}'] = feature
        elif feature_type == 'daylight':
            for index, row in df.iterrows():
                smiles = row[smiles_col]
                features = smiles2daylight(smiles)
                if features is not None:
                    for i, feature in enumerate(features):
                        df.at[index, f'daylight_{i}'] = feature
...
def smiles2atompairs(s, nBits=2048):
    mol = Chem.MolFromSmiles(s)
    if mol is None:
        return None
    return np.array(rdMolDescriptors.GetHashedAtomPairFingerprintAsBitVect(mol, nBits=nBits))

def smiles2daylight(smiles, NumFinger=2048):
    from rdkit import Chem
    from rdkit.Chem import AllChem

    molecule = Chem.MolFromSmiles(smiles)
    if molecule is None:
        return None  # Handle cases where the SMILES string could not be parsed

    bv = AllChem.GetMorganFingerprintAsBitVect(molecule, radius=2, nBits=NumFinger)
...
def add_features(df, smiles_col='SMILES', feature_types=None):
    """
    Add specified molecular features to the DataFrame based on SMILES strings.
    
    Parameters:
    - df: DataFrame containing the SMILES strings.
    - smiles_col: Column name for SMILES strings.
    - feature_types: List of feature types to include (e.g., ['morgan', 'pubchem', 'rdkit2d']).
    ...
    # Generate physical and chemical properties
    if 'physchem' in feature_types:
        physchem_features = generate_physchem_features(df[smiles_col])
        df = pd.concat([df, physchem_features], axis=1)

    # Generate molecular fingerprints and other descriptors
    for feature_type in feature_types:
        if feature_type != 'physchem':  # Physchem is already handled
            df = append_molecular_descriptors(df, smiles_col, feature_type)

    return df

def append_molecular_descriptors(df, smiles_col, descriptor_type):
    """
    Helper function to compute and append molecular descriptors to the DataFrame.
    """
    descriptor_funcs = {
        'morgan': smiles2morgan_raw,
        'pubchem': smiles2pubchem,
        'rdkit2d': smiles2rdkit2d,
        'maccs': smiles2maccs,
        'daylight': smiles2daylight
    }
    descriptor_func = descriptor_funcs[descriptor_type]

    # Compute features for each SMILES string
    for index, row in df.iterrows():
        smiles = row[smiles_col]
        features = descriptor_func(smiles)
        if features is not None:
            for j, feature in enumerate(features):
                df.at[index, f'{descriptor_type}_{j}'] = feature
...
def sp(splitter, dataset_ex):
    try:
        _, valid_dataset_ex, test_dataset_ex = splitter.train_valid_test_split(dataset_ex)
        print("Dataset successfully split without any AtomValenceException.")
    except Exception as e:
        pass
